...
function lint(string) {
  ...
  const mixForm = findObjectsMix(ast, "form");
  console.log(mixForm);

/*
if(mixForm) {
  validateInputSizes(mixForm);
  validateContentSpaces(mixForm);
  validateContentItem(mixForm);
  validateHeaderFooterText(mixForm, "header");
  validateHeaderFooterText(mixForm, "footer");
  validateHeaderFooterSpaces(mixForm, "header");
  validateHeaderFooterSpaces(mixForm, "footer");  
}*/

return errors;


  function findObjectsMix(item, name) {
    let newObj = {...item},
      mix = false,
      soughtObject = false;
    ...
    function findObj(item, name) {      

      if (item.type === 'Property') {
        if(item.key.value === "mix") mix = true;
        if (mix && typeof name === "string") {
          let arr = [];
          arr.push(name);
          name = arr;
          ...
          if (namesResult.length && mix) {
            soughtObject = item;
            return;
          }
        }
      }
      else if (mix && item.value.children) {
        item.value.children.forEach(child => {
          findObj(child, name);
          if (soughtObject) {
            for(prop in item) {
              newObj[prop] = soughtObject[prop]
            }
            soughtObject = false;
            mix = false;
            return;
